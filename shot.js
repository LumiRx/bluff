...
(async () => {
  ...
  await p.click('#bk'); await p.click('#prof'); await p.waitForSelector('#pset'); await p.waitForTimeout(200);
  await p.screenshot({path:'s7-profile.png'});
  await p.click('#pset'); await p.waitForSelector('#wipe'); await p.waitForTimeout(200);
  await p.screenshot({path:'s7b-settings.png'});
