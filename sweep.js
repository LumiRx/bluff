...
function offerTells(word,strictVoid){
...
const STRATS={
 'BAIT   sharp card, crowded pin':(c,sv)=>{const w=c[0],o=offerTells(w,sv);
   const b=o.filter(t=>t.k==='pin'&&t.n>=14&&t.n<=CAP[0]);
   const t=b.length?b[(Math.random()*b.length)|0]:o[0];return[w,t,capFor(t.n)]},
 'HIDE   sharp card, thinnest tell':(c,sv)=>{const w=c[0],o=offerTells(w,sv);
   const t=o[o.length-1];return[w,t,capFor(t.n)]},
 'RANDOM any card, any tell':(c,sv)=>{const w=c[(Math.random()*5)|0],o=offerTells(w,sv);
   const t=o[(Math.random()*o.length)|0];
   return[w,t,Math.min([25,50,100][(Math.random()*3)|0],capFor(t.n))]},
 'NAIVE  soft card, big pin':(c,sv)=>{const w=c[4],o=offerTells(w,sv);
  ...
  for(const [nm,fn] of Object.entries(STRATS)){
    ...
    for(let h=0;h<N;h++){
      ...
      for(let s=0;s<5;s++){const b=mkBot();
        ...
        const bar=.5+b.tight+(stake-50)/100*.06;
        if(est<=bar){cn-=ANTE;continue}
        calls++;
        if(solveOnce(word,pool.slice(),b.skill,G)>0){cracks++;hand-=stake;cn+=stake-ANTE}
