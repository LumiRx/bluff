...
function qrMatrix(text,forceV){
  ...
  const applyFmt=(g,mask)=>{
    ...
    for(let i=0;i<=5;i++){g[8][i]=(f>>i)&1;g[size-1-i][8]=(f>>i)&1;}
    g[8][7]=(f>>6)&1;g[8][8]=(f>>7)&1;g[7][8]=(f>>8)&1;
    g[size-7][8]=(f>>6)&1;
    for(let i=9;i<15;i++)g[14-i][8]=(f>>i)&1;
    for(let i=7;i<15;i++)g[8][size-15+i]=(f>>i)&1;   /* bit 7 sits at (8,size-8) */
    g[size-8][8]=1;
  };
  const penalty=g=>{
    let p=0;
